...
require("../npm/dayjs/locale/zh-cn.js");
...
exports.default = _dayjsMin2.default;
function formatTimestamp(timestamp, id) {
  var curTime = (0, _dayjsMin2.default)();
  var curWeek = curTime.day();
  var curDay = curTime.date();
  var curYear = curTime.year();
  var curMonth = curTime.month();
  var time = (0, _dayjsMin2.default)(timestamp);
  var week = time.day();
  var day = time.date();
  var year = time.year();
  var month = time.month();
  var weekfirstday = curTime.valueOf() - 86400000 * (curWeek - 1);
...
function getRelativeTime(timestamp) {
  try {
    ...
    if (curDay === day || curDay - 1 === day) {
      var curTimeValue = (0, _dayjsMin2.default)(timestamp).fromNow();
      return curTimeValue === '昨天' ? curTimeValue + " " + time.format('HH:mm') : time.format('HH:mm');
    }
    if (curTime.valueOf() - timestamp <= 604800000) return time.format('ddd HH:mm');
